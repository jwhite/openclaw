...
export type VoiceDiaryLogger = {
  warn: (message: string) => void;
};

export type WriteVoiceDiaryEntryParams = {
  mempalaceUrl: string;
  said: string;
  response: string;
  deviceId?: string;
  timestampMs: number;
  logger: VoiceDiaryLogger;
};

export function formatDiaryContent(params: WriteVoiceDiaryEntryParams): string {
...
export async function writeVoiceDiaryEntry(params: WriteVoiceDiaryEntryParams): Promise<void> {
  ...
  try {
    await Promise.race([
      ...
      new Promise<never>((_, reject) =>
        setTimeout(() => reject(new Error("MemPalace connect timed out")), CONNECT_TIMEOUT_MS),
      ),
    ]);
