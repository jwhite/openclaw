...
import { extractSpokenTextFromPayloads, SPOKEN_OUTPUT_CONTRACT } from "./spoken-text.js";
/**
...
export async function generateHaVoiceResponse(params) {
    ...
    try {
        return await agentRuntime.session.runWithWorkAdmission({ storePath, sessionKey }, async (abortSignal) => {
            ...
            const now = Date.now();
            const existingSessionEntry = agentRuntime.session.getSessionEntry({ storePath, sessionKey });
            ...
            if (!sessionEntry?.sessionId) {
                sessionEntry =
                    (await agentRuntime.session.patchSessionEntry({
                        ...
                        update: (entry) => entry.sessionId ? entry : { ...entry, sessionId: crypto.randomUUID(), updatedAt: now },
                    })) ?? undefined;
            }
            if (!sessionEntry?.sessionId) {
