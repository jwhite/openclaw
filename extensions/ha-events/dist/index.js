...
import { definePluginEntry, resolveConfiguredSecretInputString } from "./api.js";
import { resolveHaEventsPluginConfig } from "./src/config.js";
import { createEmptySnapshot, recordConnected, recordDisconnected, recordEvent, } from "./src/event-store.js";
...
export function getHaEventsSnapshot() {
    return snapshot;
}
async function connectHaEventsClient(api, resolved) {
    ...
    connectStarted = true;
    const token = typeof resolved.token === "string"
        ? resolved.token
...
export default definePluginEntry({
    ...
    description: "Subscribes to Home Assistant's state_changed events over WebSocket, with reconnect/backoff.",
    register(api) {
